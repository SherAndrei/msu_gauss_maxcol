...
// A_{i, j} size: av * ah
static inline void print_matrix(const double* A, const int v, const int h,
                                const int m, const int r) {
    // количество печатаемых элементов в ширину
    const int nv = MIN(v, r);
    // количество печатаемых элементов в высоту
    const int nh = MIN(h, r);
    ...
    const int lh = nh - kh * m;
    const int lv = nv - kv * m;
